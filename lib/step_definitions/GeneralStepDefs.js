...
defineSupportCode(function({Given, When, Then, setWorldConstructor}) {
  throw new Error('this is loaded');
  ...
  //});
  //this.When(/^I click the "([^"]*)"(?: )?(link|button|drop down list|tab|)$/, function(elementName, elementType) {
  //  var element;
  ...
  //});
  //this.Then(/^the "([^"]*)" should be present$/, function(el) {
  ...
  //});
  //return this.Then(/^the "([^"]*)"(?: )?(checkbox|) (should|should not) be checked$/, function(el, elType, expectation) {
  //  var elementType;
  //  elementType = this.transform.elementTypeToVariableName(elType);
  //  this.el = this.transform.stringToVariableName(el + elementType);
  //  this.expectation = this.transform.shouldToBoolean(expectation);

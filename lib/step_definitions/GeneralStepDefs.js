...
defineSupportCode(function({Given, When, Then, setWorldConstructor}) {
  ...
  Then('the title should equal "{text:captureString}"', function(text) {
  ...
  Then('the "{name:elementName}"{type:elementType} should be present', function(name, type) {
    var element = name + type;
    return this.expect(this.currentPage[element].isPresent()).to.eventually.equal(true);
  });
  ...
  Then('(the |)"{name:elementName}"{type:elementType} should (have|contain) the text "{text:captureString}"', function(name, type, text) {
  ...
  Then('"{option:captureString}" should appear in the "{name:elementName}"{type:elementType}', function(option, name, type) {
